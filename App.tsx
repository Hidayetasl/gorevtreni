...
import { ActiveWriterDevice, TabType, RoutineTask, ShopItem, PlacedWorldItem, UserProfile, ParentConfig, BonusCard, VoiceMessage, StoryVideo, ActivityLogEntry, CoinLedgerEntry } from './types';
import {
  getStoredTasks,
  saveStoredTasks,
  getStoredShop,
...
import { acceptFamilyInvite, createFamilyCode, familyExists, getFamilyCode, getFamilyData, getInviteFamilyCode, isCloudConfigured, mergeById, saveFamilyCode, subscribeToFamily, uploadFamilyData } from './utils/cloudSync';
...
export default function App() {
  ...
  const [activeWriterDevice, setActiveWriterDevice] = useState<ActiveWriterDevice | null>(() => (
    !isCloudConfigured && deviceRole === 'writer'
      ? { deviceId, label: getBrowserDeviceLabel(), setAt: new Date().toISOString() }
      : null
  ));
  const isActiveWriter = activeWriterDevice?.deviceId === deviceId;
  // Gerçek yazma yetkisi her durumda tek bir kaynağa bağlıdır: aktif cihaz
  // kaydındaki deviceId. İlk kurulumda bu kayıt yerelde hazırlanır ve aile
  // oluşturulurken buluta taşınır.
  const canWrite = isActiveWriter;
  const isViewer = !canWrite;
  const [hasGameAccess, setHasGameAccess] = useState(() => localStorage.getItem('ruzgar_game_access_v1') === 'open' && (!isCloudConfigured || getStoredDeviceRole() !== null));
  ...
  }, [user, parentConfig, tasks, shop, world, bonuses, voiceMessages, videos, activityLog, coinLedger, activeWriterDevice]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        else if (isViewer) throw new Error('Bu aile kaydı bulunamadı; önce ana cihazda kurulum yapılmalı.');
        ...
        unsubscribe = await subscribeToFamily(familyCode, (remote, metadata) => {
          ...
          const hasActiveWriterField = Object.prototype.hasOwnProperty.call(remote, 'activeWriterDevice');
          // Eski aile belgeleri alanı içermiyorsa, ilk writer cihazı güvenli geçiş
          // için tek seferlik aktif cihaz olarak başlatılır.
          if (!hasActiveWriterField && deviceRole === 'writer') {
            setActiveWriterDevice({ deviceId, label: getBrowserDeviceLabel(), setAt: new Date().toISOString() });
          } else {
            setActiveWriterDevice(remote.activeWriterDevice || null);
          }
          const syncedLedger = isViewer ? (remote.coinLedger || []) : mergeById(remote.coinLedger || [], coinLedger);
          ...
          const localWorld = isViewer ? [] : (latestFamilyDataRef.current?.world || []);
          ...
          setTasks(isViewer ? (remote.tasks || []) : mergeById(remote.tasks || [], tasks));
          setShop(isViewer ? syncedShop : mergeById(syncedShop, shop)); setWorld(syncedWorld); setBonuses(isViewer ? (remote.bonuses || []) : mergeById(remote.bonuses || [], bonuses));
          setCoinLedger(syncedLedger);
          const remoteMessages = remote.voiceMessages || [];
          const localMessages = isViewer ? [] : voiceMessagesRef.current;
          ...
          const localVideos = isViewer ? [] : videosRef.current;
          ...
          const localActivityLog = isViewer ? [] : activityLogRef.current;
          ...
          if (!isViewer && (shopCatalogChanged || worldChanged || combinedVideos.length > remoteVideos.length || combinedMessages.length > remoteMessages.length || combinedActivityLog.length > remoteActivityLog.length)) {
  ...
  }, [familyCode, cloudEnabled, networkEpoch, isViewer]);

  useEffect(() => {
    if (isViewer || !cloudEnabled || !isCloudConfigured || !familyCode || !syncReadyRef.current || remoteUpdateRef.current) return;
    if (!navigator.onLine) {
  ...
  }, [user, parentConfig, tasks, shop, world, bonuses, voiceMessages, videos, activityLog, coinLedger, familyCode, cloudEnabled, isViewer]);
  ...
  const handleManualSync = async () => {
    if (isViewer) {
      setCloudStatus('İzleyici modu: bulut verisi yalnızca okunuyor.');
      return;
    }
    if (!familyCode) {
  ...
  const handleCreateFamily = async () => {
    if (deviceRole === 'viewer') throw new Error('İzleyici cihazdan yeni aile oluşturulamaz.');
    const code = createFamilyCode();
    const initialActiveWriter: ActiveWriterDevice = { deviceId, label: getBrowserDeviceLabel(), setAt: new Date().toISOString() };
    setActiveWriterDevice(initialActiveWriter);
    setFamilyCode(code);
    setCloudStatus('Aile oluşturuluyor…');
    try {
      await uploadFamilyData(code, { ...currentFamilyData(), activeWriterDevice: initialActiveWriter });
      syncReadyRef.current = true;
      setCloudStatus('Eşitlendi ✓');
  ...
  const handleSetActiveWriterDevice = async (next: ActiveWriterDevice | null) => {
    const resolved = next
      ? { ...next, deviceId, label: next.label || getBrowserDeviceLabel(), setAt: new Date().toISOString() }
      : null;
    setActiveWriterDevice(resolved);
    setCloudStatus(resolved ? 'Aktif cihaz değişikliği buluta yazılıyor…' : 'Aktif cihazlıktan çıkış buluta yazılıyor…');
    if (!isCloudConfigured || !familyCode) return;
    pendingSyncRef.current = true;
    try {
      await uploadFamilyData(
        familyCode,
        { ...currentFamilyData(), activeWriterDevice: resolved },
        { forceActiveWriterDevice: true },
      );
      pendingSyncRef.current = false;
      setCloudStatus(resolved ? 'Bu cihaz aktif yapıldı; diğer cihazlar yalnızca izler.' : 'Aktif cihazlıktan çıkıldı.');
    } catch (error) {
      setCloudStatus(`Eşitleme hatası: ${getCloudErrorMessage(error, 'Aktif cihaz değiştirilemedi.')}`);
    }
  };

  const handleJoinFamily = async (code: string) => {
    if (isViewer) throw new Error('İzleyici cihazdan aile ayarları değiştirilemez.');
  ...
  if (!hasGameAccess) {
    ...
    return (
        <SimpleAccessGate
          onUnlock={(code, role, familyData) => {
            if (code) setFamilyCode(code);
            if (role) {
              setDeviceRole(role);
              if (role === 'writer') setActiveWriterDevice({ deviceId, label: getBrowserDeviceLabel(), setAt: new Date().toISOString() });
              else setActiveWriterDevice(null);
            }
            // Katılım başarıyla tamamlandıysa erişim ekranından çıkmadan önce
            // Firestore snapshot'ını yerel başlangıç state'inin üzerine koy.
            // Böylece ilk render 6 coin/9 görevli yerel profili göstermeden
            // doğru aile profiliyle başlar; abonelik yine arka planda sürer.
            if (familyData) {
              ...
              setCoinLedger(syncedLedger);
              setActiveWriterDevice(familyData.activeWriterDevice || null);
            }
    ...
    <div className="app-shell min-h-screen bg-[#F9F6F0] text-[#4E342E] relative selection:bg-[#FFF59D]">
      ...
      <div className="relative z-10 max-w-5xl mx-auto flex flex-col min-h-screen">
        ...
        <Header
          ...
          hasUnclaimedBonus={!isViewer && !!unclaimedBonus}
          onOpenBonusModal={!isViewer ? () => {} : undefined}
          pendingCount={pendingCount}
          onOpenVoiceModal={activeTab === 'tasks' ? undefined : () => openVoiceModal('inbox')}
          unreadVoiceCount={unreadVoiceCount}
          cloudStatus={cloudStatus}
          onManualSync={isViewer ? undefined : handleManualSync}
        ...
        <main className="app-main flex-1 p-2 sm:p-3">
          {activeTab === 'tasks' && (
            <TasksView
              ...
              readOnly={isViewer}
              onOpenVoiceModal={isViewer ? undefined : openVoiceModal}
              onOpenJournal={isViewer ? undefined : openJournal}
              unreadVoiceCount={unreadVoiceCount}
            />
          )}
        ...
        <ParentModal
          ...
          activeWriterDevice={activeWriterDevice}
          currentDeviceId={deviceId}
          onSetActiveWriterDevice={handleSetActiveWriterDevice}
